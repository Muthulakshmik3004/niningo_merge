...
export default function LocationScreen() {
  ...
  const isValidAddress = (value: string) => {
    ...
    return alnumCount / trimmed.length >= 0.4;
  };
  ...
  const handleContinue = async () => {
    ...
    if (!homeCoords && !isValidAddress(home)) {
      setHomeError("Please enter a valid address or use current location.");
      return;
    }

    if (office.trim() && !officeCoords && !isValidAddress(office)) {
      setOfficeError("Please enter a valid office address or use current location.");
      return;
    }
    ...
    >
      ...
      >
        ...
        >
          ...
          {!!homeError && (
            <Text className="text-red-500 text-[13px] mt-[6px] ml-[2px]">{homeError}</Text>
          )}
        ...
        >
          ...
          {!!officeError && (
            <Text className="text-red-500 text-[13px] mt-[6px] ml-[2px]">{officeError}</Text>
          )}
