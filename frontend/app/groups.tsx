...
useFocusEffect(
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setError(e?.message || "Could not load groups from the server");
    } finally {
      setLoading(false);
    }
  }, []);
...
onPress={() => {
  ...
  item.name.toLowerCase().includes(search.toLowerCase())
          ...
          ListEmptyComponent={
            <Text className="text-center text-[15px] text-[#777] mt-[30px]">No groups yet</Text>
          }
        renderItem={({ item }) => (
